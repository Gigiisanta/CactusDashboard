...
from cactus_wealth.models import User, ModelPortfolioPosition
...
def get_model_portfolio_repository(session: Session = Depends(get_session)) -> ModelPortfolioRepository:
    """Dependency to get model portfolio repository."""
    return ModelPortfolioRepository(session)
...
def get_model_portfolios(
    skip: int = 0,
    limit: int = 100,
    portfolio_repo: ModelPortfolioRepository = Depends(get_model_portfolio_repository),
    current_user: User = Depends(get_current_user),
) -> list[ModelPortfolioRead]:
    """
    Get all model portfolios with their positions.
    """
    portfolios = portfolio_repo.get_all_with_positions(skip=skip, limit=limit)
...
def create_model_portfolio(
    ...
    """
    try:
        portfolio = portfolio_repo.create_model_portfolio(
...
def get_model_portfolio(
    ...
    """
    portfolio = portfolio_repo.get_with_positions(portfolio_id=portfolio_id)
    if not portfolio:
        raise HTTPException(
...
def add_portfolio_position(
    ...
    try:
        ...
        current_total = portfolio_repo.total_weight(portfolio_id=portfolio_id)
        ...
        position = portfolio_repo.create_position(
...
def update_portfolio_position(
    ...
    try:
        ...
        if position_update.weight is not None:
            ...
            current_position = portfolio_repo.session.get(ModelPortfolioPosition, position_id)
        ...
        position = portfolio_repo.update_position(
...
def delete_portfolio_position(
    ...
    """
    position = portfolio_repo.delete_position(position_id=position_id)
    if not position:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Position not found"
        )
...
def validate_portfolio_weights(
    ...
    total_weight = portfolio_repo.total_weight(portfolio_id=portfolio_id)
