"""
WebAuthn/Passkey authentication endpoints exposed under
`cactus_wealth.api.v1.endpoints.passkeys` for tests and API.

This module mirrors the behavior of the auth passkeys endpoints
but lives in the expected import path used by tests.
"""
...
async def get_registration_options(
    request: RegistrationOptionsRequest,  # noqa: ARG001
    ...
    try:
        ...
        return [
            CredentialRead(
                ...
                last_used_at=getattr(cred, "last_used_at", None),
            )
...
async def delete_credential(
    ...
    try:
        webauthn_service = WebAuthnService(session)
        user_id = cast(int, current_user.id)
        success = await webauthn_service.delete_credential(credential_id, user_id)
